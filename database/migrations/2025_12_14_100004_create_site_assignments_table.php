...
/**
 * Create the site_assignments table for flexible user-to-site role assignments.
 *
 * This table enables tenant-specific role names (e.g., "Site Manager", "Operations Lead")
 ...
 * - Unique constraint prevents duplicate user+role assignments to same site
...
return new class extends Migration
{
    ...
    {
        Schema::create('site_assignments', function (Blueprint $table): void {
            ...
            // Examples: "Account Manager", "Site Manager", "Operations Lead", "Quality Manager"
            ...
            $table->unique(['site_id', 'user_id', 'role'], 'unique_site_user_role');
            $table->index(['tenant_id', 'user_id'], 'idx_site_assignments_tenant_user');
            $table->index(['tenant_id', 'role'], 'idx_site_assignments_tenant_role');
            $table->index(['site_id', 'valid_from', 'valid_until'], 'idx_site_assignments_validity');

            $table->comment('Flexible user-to-site role assignments with temporal tracking');
        });
    }
