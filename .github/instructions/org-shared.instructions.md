...
- Use `secpal.app` only for the public homepage and real email addresses, `api.secpal.dev` for the API, `app.secpal.dev` for the PWA/frontend, and `secpal.dev` for dev, staging, testing, and examples. Treat `api.secpal.app` and `app.secpal.app` as deprecated web hosts; `app.secpal.app` remains valid only as the Android application identifier.
