...
> This file ensures org-wide principles are active when working in the `api` repo.
...
| **Convention > Config** | Follow framework conventions (Laravel naming, PSR-12).                                   |
| **Quality > Speed**     | Fast but broken code creates technical debt.                                             |

## 🏗 Architecture (Separation of Concerns)

```
Request → Controller (thin) → Service → Repository → Model
                           ↓
...
Controllers only orchestrate — no business logic. Services own the logic. Repositories own data access.

## 🔐 Data Protection (GDPR/DSGVO) — Critical
...
| `*_idx`   | Blind index (sha256 hash)     | Search/query only                               |
| `*_plain` | Transient write-only property | **ALWAYS** use in tests, factories, controllers |
...
| Layer    | Technology                                                          |
| -------- | ------------------------------------------------------------------- |
| Backend  | PHP 8.4, Laravel 12, DDEV (`ddev exec`), Pest v4, PostgreSQL 16     |
| Frontend | Node 22, React, TypeScript (strict), Vite, Vitest                   |
| API      | OpenAPI 3.1, REST, JSON, Bearer tokens, URL versioning (`/api/v1/`) |

> DDEV: Always use `ddev exec php artisan …` — never run `php artisan …` directly outside DDEV.
