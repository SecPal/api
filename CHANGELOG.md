...
- blocked users from downgrading or deleting their own last `admin` path on an organizational unit via `/v1/organizational-units/{organizational_unit}/scopes/{scope}`, so scope-management changes no longer allow accidental self-lockout from the very unit whose scopes they are editing
