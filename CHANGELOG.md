...
- isolate the flaky OpenTimestamp-related Merkle batch tests from real OTS submissions by faking queue dispatches broadly and stubbing `OpenTimestampService` in the affected test suites, so CI no longer depends on external calendar network timing
