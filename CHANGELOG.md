...
- rejected new organizational scopes, employee or site placements, and site-user assignments on organizational units marked `is_assignable: false`, including reassignment attempts, while preserving the required creator bootstrap `manage` scope when creating an unassignable unit (fixes `api#1266`)
