...
- replaced mutable-tag promotion in the post-merge API image publisher with a
  ...
  attestation, rechecks the tag and attestation as a final snapshot, and records
  `ghcr.io/secpal/api@sha256:<digest>` as the only canonical image identity
...
- added synchronous Activity-backed Legal Hold lifecycle audits with atomic
  ...
  categories, and explicit rollback/fail-closed audit semantics (fixes `api#454`)
...
- added a closed versioned application security-event stream for authentication,
  ...
  bounded amplification, strict parser fixtures, and privacy regression coverage
  (fixes `api#1412`)
- added a separate post-merge GHCR publisher bound exclusively to `ghcr.io/secpal/api`, with one non-authoritative run-scoped discovery tag per build and the verified OCI index digest as the only canonical identity across `linux/amd64` and `linux/arm64`; the publisher rejects Docker Hub and shortened official-image identities, preserves workflow-wide per-commit concurrency, deterministic OCI metadata, commit-addressed Git build contexts, complete SLSA v1 source-material verification, BuildKit SPDX SBOMs, max-mode SLSA provenance, digest-only remote verification, complete UID/GID-aware runtime smoke coverage using digest-pinned PostgreSQL and Valkey services, GitHub Artifact Attestation after technical and runtime verification, a final run-tag digest snapshot, least-privilege job boundaries, QEMU-before-Buildx initialization, pinned validation and privileged build-helper images, warning-free artifact-storage behavior, registry writes limited to the build and attestation actions, host-independent runtime configuration modes, and an explicit one-time public-package follow-up
