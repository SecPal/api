...
- scoped the employee-deletion causer-context backfill to the deleted employee's own organizational unit so it can no longer stamp a foreign organizational-unit or management-level snapshot onto the same user's activities in other units or global activities, mirroring the write-time capture invariant and keeping activity-log index and detail visibility consistent
