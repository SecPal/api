...
- stopped synthesizing missing activity causer rank snapshots during employee deletion, so legacy activity rows without a trustworthy per-event snapshot are no longer stamped with deletion-time employee state; employee deletion now preserves shared legacy user accounts for active employee links while deprovisioning shared users that are only retained by trashed employee references, including randomizing their primary credentials and anonymizing the retained login identifier so fresh session/token logins cannot be reissued
