...
- made the `2026_04_12_150000_add_non_eu_work_permit_core_to_employees_table` migration rewrite work-permit enum values safely on SQLite by temporarily relaxing the legacy enum check, restoring the updated check after the rewrite, preserving the rollback rewrite path, and keeping the existing PostgreSQL constraint flow intact for Polyscope and other SQLite-backed setup/migration paths (refs `api#1227`)
