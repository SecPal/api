...
- **Breaking:** removed the orphaned Android enrollment-session, provisioning QR, and bootstrap-token exchange API surfaces. The `android_enrollment.*` permissions and `managed_android_enrollment` bootstrap flag are gone; the enrollment-session table is irreversibly dropped without retaining obsolete bootstrap-token material (refs `api#1353`).
