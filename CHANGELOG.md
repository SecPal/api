...
- hardened OpenTimestamp proof verification so a quorum of at least two canonical HTTPS API origins must agree on the attested height, a valid header can be fetched from any agreeing origin, the fetched header must hash to that block, and its Merkle root must match the attestation before a proof is accepted; Laravel now passes config-cached provider settings explicitly, API responses are bounded, malformed providers fail cleanly, and successful cache decisions are bound to the exact proof and provider configuration so vulnerable or stale results are never reused
