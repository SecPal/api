...
- added a separate post-merge GHCR publisher bound exclusively to `ghcr.io/secpal/api` for immutable full-commit-SHA API image tags and canonical OCI index digests across `linux/amd64` and `linux/arm64`, with fail-closed rejection of Docker Hub and shortened official-image identities, fail-closed tag-existence checks and digest reuse on workflow reruns, exact OCI metadata, BuildKit SPDX SBOMs, max-mode SLSA provenance, OIDC-backed GitHub Artifact Attestations, digest-only remote and runtime verification including UID/GID, least-privilege job boundaries, digest-pinned container validation tools, a single policy-guarded registry-write path, host-independent runtime configuration modes, and an explicit one-time public-package follow-up
