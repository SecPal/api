...
- hardened OpenTimestamp proof verification so at least two distinct Bitcoin header API bases must agree on the attested height, the fetched header must hash to that block, and its Merkle root must match the attestation before a proof is accepted; verification now uses a shared network deadline and a versioned cache namespace so results produced by the vulnerable verifier are never reused
