...
- made tenant KEK setup safe under parallel Pest workers by introducing `TenantKey::ensureKekExists()`, a race-tolerant idempotent variant of `generateKek()` that swallows the harmless `fopen('xb')` "File exists" race when concurrent workers reach the shared `storage/app/keys/kek.key` path at the same time. Test factories (`TenantKeyFactory`, `EmployeeFactory`, `CustomerFactory`, etc.), seeders, and test setup blocks now call `ensureKekExists()` instead of the racy `if (! file_exists) generateKek()` pattern that previously caused `php artisan test --parallel` to fail with `fopen(...): Failed to open stream: File exists`. Production `generateKek()` retains its strict refusal to overwrite an existing KEK; a `pcntl_fork`-based regression test creates 8 concurrent workers that race on the same KEK path and verifies all of them succeed (refs `api#1106`)
