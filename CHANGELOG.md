...
- customer and site create/delete activity entries now embed the entity type, canonical number, and display name directly in the searchable `description` and mirrored `properties` metadata, so the existing `GET /v1/activity-logs?search=...` surface can reliably find create/delete history by customer name, site name, or identifier without any frontend contract change (fixes `api#1213`)
