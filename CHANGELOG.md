...
- skipped PostgreSQL-only work-permit check-constraint DDL in the `2026_04_12_150000_add_non_eu_work_permit_core_to_employees_table` migration when running against SQLite, preserving the existing PostgreSQL constraint rewrite flow while allowing Polyscope and other SQLite-backed setup/migration paths to progress through that rollout (refs `api#1227`)
