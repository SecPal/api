...
- added a separate post-merge GHCR publisher bound exclusively to `ghcr.io/secpal/api` for immutable full-commit-SHA API image tags and canonical OCI index digests across `linux/amd64` and `linux/arm64`, with fail-closed rejection of Docker Hub and shortened official-image identities, content-negotiation-safe detection of existing OCI and Docker manifests before first publication, per-commit concurrency, retry-safe reuse of existing digests without moving their tags, exact deterministic OCI metadata, commit-addressed Git build contexts, complete SLSA v1 source-material verification, BuildKit SPDX SBOMs, max-mode SLSA provenance, post-verification OIDC-backed GitHub Artifact Attestations for the selected digest, digest-only remote verification and complete UID/GID-aware runtime smoke coverage for both platform manifests, least-privilege job boundaries, QEMU-before-Buildx initialization, digest-pinned validation, SBOM scanner, and privileged build-helper images, explicit warning-free artifact-storage behavior, a single policy-guarded registry-write path, host-independent runtime configuration modes, and an explicit one-time public-package follow-up
