...
- added a separate post-merge GHCR publisher for immutable full-commit-SHA API image tags and canonical OCI index digests across `linux/amd64` and `linux/arm64`, with exact OCI metadata, BuildKit SPDX SBOMs, max-mode SLSA provenance, OIDC-backed GitHub Artifact Attestations, digest-only remote and runtime verification, least-privilege job boundaries, static publishing-policy coverage, and an explicit one-time public-package follow-up
