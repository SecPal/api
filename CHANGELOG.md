...
- added a separate post-merge GHCR publisher bound exclusively to `ghcr.io/secpal/api` for immutable full-commit-SHA API image tags and canonical OCI index digests across `linux/amd64` and `linux/arm64`, with fail-closed rejection of Docker Hub and shortened official-image identities, content-negotiation-safe detection of existing OCI and Docker manifests, workflow-wide per-commit concurrency, unique internal candidate tags, complete candidate verification before OIDC-backed GitHub Artifact Attestation, a harmless runtime probe that requires GHCR to enforce create-only conditional manifest writes before exact-byte digest-preserving promotion to the final SHA tag, rejection of existing final tags without a matching trusted attestation, deterministic OCI metadata, commit-addressed Git build contexts, complete SLSA v1 source-material verification, BuildKit SPDX SBOMs, max-mode SLSA provenance, digest-only remote verification and complete UID/GID-aware runtime smoke coverage for both platform manifests using digest-pinned PostgreSQL and Valkey services, least-privilege job boundaries, QEMU-before-Buildx initialization, digest-pinned validation, SBOM scanner, and privileged build-helper images, explicit warning-free artifact-storage behavior, tightly bounded registry-write paths, host-independent runtime configuration modes, and an explicit one-time public-package follow-up
...
- added the canonical authorized domain lookup cascade (`/v1/lookups/legal-entities` → Legal Entity establishments → establishment customers) with minimal responses, and centralized existing tenant, RBAC, organizational-scope, customer-assignment, and site-assignment checks behind one internal domain-access layer shared by customer and customer-establishment lookup, list, create, and update paths; site-scoped users no longer see other establishment links of the same customer (US-004).
- added customer master-data duplicate protection and customer-establishment management: PostgreSQL now atomically enforces normalized VAT-ID and normalized name-plus-full-billing-address uniqueness per tenant and Legal Entity, every duplicate path returns the same neutral conflict response, and `/v1/customer-establishments` provides tenant-safe CRUD for encrypted local contact data without exposing organizational-unit data (US-003).
...
- **Breaking:** replaced the unreleased customer-specific `GET /v1/customers/legal-entities` lookup with the canonical `/v1/lookups/*` domain cascade defined by the shared contract.
...
- existing final API image tags now require their registry-backed GitHub Artifact Attestation to verify for the expected repository, workflow, source branch, commit, image name, and digest before either runtime manifest is inspected, pulled, or executed; candidate verification remains before candidate attestation and promotion
- notification registration now accepts only strict integer runtime schema `4`, matching the canonical bootstrap response for Android FCM and Web Push; schema `3` and all other unsupported or malformed values fail validation without persistence (fixes `api#1360`; supersedes `api#1359` under `SecPal/.github#590`).
- **Breaking:** removed the orphaned Android enrollment-session, provisioning QR, and bootstrap-token exchange API surfaces. The `android_enrollment.*` permissions and `managed_android_enrollment` bootstrap flag are gone, the bootstrap schema revision advances from `3` to `4`, fresh installations no longer create the obsolete enrollment-session table or retain bootstrap-token material, and upgrades preserve immutable enrollment audit records without requiring the removed subject model (refs `api#1353`).
- synchronized Laravel locale changes with GNU Gettext's `LANGUAGE` selector, isolated the default locale between tests, made OpenTimestamp temporary-file assertions honor the runtime temp directory, and aligned `composer test` with the supported parallel/serial test split so complete validation remains deterministic in sanitized environments.
- customer list, create, detail, and update responses now always embed the caller-visible `customer_establishments` collection, including an empty array when no links exist; site-assigned callers only receive links matching their active site assignments.
- hardened the Legal Entity and establishment rollout so site and assignment creation plus site coverage expansion reject inactive domain links, customer-establishment writes require active Customer, Legal Entity, and establishment records, scoped customer and employee readers ignore stale assignments from deleted domains, employee details respect tenant-isolated document and qualification policies, employee and site writes reject legacy organizational-unit input, domain lookups support the corresponding write permissions without widening scoped access, restored customer-establishment links clear omitted local contact data, and factories safely reuse soft-deleted links.
...
- hardened OpenTimestamp proof verification so exactly one quorum of at least two canonical HTTPS API origins must agree on the attested height, DNS-equivalent trailing-dot hostnames cannot count as separate origins, conflicting quorums fail closed, redirects are rejected before contacting a different origin, any configured origin can supply a header that hashes to the agreed block, and its Merkle root must match the attestation before a proof is accepted; Laravel now passes config-cached provider settings explicitly, requests, proofs, and attestation traversal have strict resource bounds, the shared deadline fairly preserves later attestations and every remaining height/header provider attempt, malformed or truncated provider responses fail cleanly without blocking healthy APIs, and successful `v4` cache decisions are proof/provider-bound, application-key authenticated with their verification time, and expire within an operator-configurable maximum of one day so database tampering or a stale store expiry cannot bypass periodic active-chain revalidation
...
- hardened multi-tenant organizational-scope and employee HR authorization so scope-assignment CRUD now requires an explicit `organizational_scopes.manage` permission in addition to `manage` unit access, self-scope widening is blocked, employee `delete`/`activate`/`terminate`/leave transitions now reuse the same tenant/scope/rank gates as `view`/`update`, salary data is withheld unless `employees.read_salary` is present and salary writes are rejected without that permission, employee deletion now securely deprovisions the linked runtime user account while preserving historical customer/site assignments and retained employee document/onboarding audit references, and self-scope responses plus OU restore handling now tolerate soft-deleted organizational units without 500s
...
- preserved employee causer rank context before employee deletion, retention deletion, or runtime-user unlinking so activity-log index and detail authorization keep enforcing organizational-scope rank limits for historical entries instead of treating deprovisioned employee users as system actors; activity rows now keep the organizational-unit and management-level snapshot in dedicated non-hashed columns so later deprovisioning does not mutate hashed `properties` payloads or break hash-chain verification
...
- normalized inherited PostgreSQL test database names before Laravel's parallel-test lifecycle applies its worker token, preventing mismatched or repeated `_test_<token>` suffixes during database recreation, preserving worker isolation for traitless tests and direct `TEST_TOKEN` workers with token-specific bootstrap env files, tracking the original database base explicitly so caller-provided names remain valid even when they use the default prefix and end in suffix-shaped values, hashing path-unsafe Laravel worker tokens consistently, restoring bootstrap-probe database and schema overrides after each token dataset, aligning preflight checks with migrations, and keeping database-outage coverage deterministic when reused worker databases contain stale public-schema tables (fixes `api#1315`)
- `GET /v1/organizational-units` now accepts textual `true` and `false` query values for the `is_active` and `is_assignable` filters in addition to the existing `1` and `0` values (fixes `api#1305`)
...
- moved `/health`, `/health/live`, and `/health/ready` onto a DB-independent health-route stack and cache-safe health throttle that preserves configured non-database stores while falling back to file cache for database-backed or unavailable stores, and now keeps shared Redis-backed throttling intact even when PhpRedis serialization or compression is enabled, so missing database credentials no longer crash public health probes inside API/session/cache middleware; `GET /health/ready` also degrades cleanly to `503 not_ready` when database connectivity or tenant-key checks throw non-`Exception` PHP throwables
- deleted SQLite-null `uploaded_by` audit rows before the `2026_06_29_120000_allow_employee_audit_records_to_survive_user_deletion` rollback rebuild restores `NOT NULL`, so SQLite rollback now matches the PostgreSQL cleanup path instead of aborting on preserved historical rows
- made the `2026_04_12_150000_add_non_eu_work_permit_core_to_employees_table` migration rewrite work-permit enum values safely on SQLite by rebuilding only the `work_permit_type` check while preserving the table's other SQLite enum checks, preserving the rollback rewrite path, and keeping the existing PostgreSQL constraint flow intact for Polyscope and other SQLite-backed setup/migration paths (refs `api#1227`)
...
- **Breaking:** activity-log hash verification now uses the single canonical payload that includes the preserved causer snapshot columns; rows hashed with the older payload no longer verify as trusted, and `activity_log.read_system` is now enforced consistently for global privileged-user audit entries while still allowing users to inspect their own privileged activity without broadening access to other system-user events
- revoked all unexchanged Android enrollment sessions (including already-expired bootstrap tokens) and deleted stale push-device registrations when the retention/GDPR expired-employee deletion flow anonymizes a linked user, closing the data-linkage path where anonymized accounts could still hold enrollment state or active notification registrations regardless of token expiry (fixes `api#1188`)
- documented and regression-tested that the retention/GDPR expired-employee deletion flow erases customer and site assignment rows outright, intentionally diverging from the admin deprovisioning path that preserves historical assignment rows with `user_id = null` (fixes `api#1189`)
...
- auto-grant a direct scope on newly created child organizational units and preserve only the actor's pre-move access level when reparenting existing units, so child-unit responses remain reload-stable without escalating permissions from the destination parent.
...
- made passkey registration verification and passkey deletion return `404` before checking the current-password step-up when the target challenge or credential is missing, taught the `passkey-verify` rate limiter to count invalid `current_password` failures, added a dedicated per-user step-up lockout so stolen-token password guessing cannot bypass the verify throttle by rotating source IPs, and normalized blank passkey user-verification config back to the secure `required` default instead of silently downgrading to `preferred`
