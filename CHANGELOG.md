...
- temporarily accepted notification-registration runtime schemas `3` and `4` while continuing to emit canonical bootstrap schema `4`; remove schema `3` only after the minimum supported Android release no longer restores or submits schema-3 runtime state, together with the frontend allowlist and Android runtime contract (refs `api#1355`).
