...
- hardened OpenTimestamp proof verification so exactly one quorum of at least two canonical HTTPS API origins must agree on the attested height, conflicting quorums fail closed, any configured origin can supply a header that hashes to the agreed block, and its Merkle root must match the attestation before a proof is accepted; Laravel now passes config-cached provider settings explicitly, individual requests have bounded time and response sizes, the shared deadline reserves time for header retrieval and one fallback after reaching quorum, malformed or truncated provider responses fail cleanly without blocking healthy APIs, and successful cache decisions are bound to the exact proof and provider configuration so vulnerable or stale results are never reused
