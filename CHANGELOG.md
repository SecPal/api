...
- hardened multi-tenant organizational-scope and employee HR authorization so scope-assignment CRUD now requires an explicit `organizational_scopes.manage` permission in addition to `manage` unit access, self-scope widening is blocked, employee `delete`/`activate`/`terminate`/leave transitions now reuse the same tenant/scope/rank gates as `view`/`update`, salary data is withheld unless `employees.read_salary` is present, employee deletion now securely deprovisions the linked runtime user account without breaking retained employee document and onboarding audit references, and self-scope responses plus OU restore handling now tolerate soft-deleted organizational units without 500s
