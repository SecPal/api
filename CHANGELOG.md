...
- stabilized PostgreSQL `RefreshDatabase` bootstrap state for the `address-data` Pest slice by clearing Laravel's stale migrated flag when a rebooted test app no longer sees the `migrations` table, and added focused regression coverage plus repeated grouped validation for the DB-backed address-data tests (fixes `api#1207`)
