...
- normalized inherited PostgreSQL test database names before Laravel's parallel-test lifecycle applies its worker token, preventing mismatched or repeated `_test_<token>` suffixes during database recreation, preserving worker isolation for traitless tests and direct `TEST_TOKEN` workers with token-specific bootstrap env files, keeping caller-provided database bases valid even when their names end in suffix-shaped values, hashing path-unsafe Laravel worker tokens consistently, aligning preflight checks with migrations, and keeping database-outage coverage deterministic when reused worker databases contain stale public-schema tables (fixes `api#1315`)
