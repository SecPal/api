...
- replaced the Prettier and Markdownlint Node hooks with pinned npm hooks, so pre-commit no longer invokes its incompatible Node environment installer with current npm versions, and added an npm 12 CI smoke test that installs and executes both hooks (fixes `api#1308`, `api#1309`)
