...
- rejected new organizational scopes, scope expansions, employee or site placements, and site-user assignments on organizational units marked `is_assignable: false`, including reassignment, reactivation, future-coverage extension, and role-change attempts plus soft-deleted targets, while preserving the required creator bootstrap `manage` scope when creating an unassignable unit (fixes `api#1266`)
