...
- public notification runtime discovery now derives shared channel-aware state from the backend bootstrap configuration: `GET /v1/bootstrap` additionally returns exhaustive `features.notification_channels` flags plus canonical `notification_channels.android_fcm` runtime metadata, while the existing Android `android_push` bootstrap alias remains available during the transition on `main`; authenticated Android push-device conflicts now fail closed with generic `NOTIFICATION_CHANNEL_UNSUPPORTED` / `NOTIFICATION_RUNTIME_STATE_INVALID` payloads keyed by channel instead of Android-only field names (refs `api#1132`)
