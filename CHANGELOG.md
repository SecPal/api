...
- added customer master-data duplicate protection and customer-establishment management: PostgreSQL now atomically enforces normalized VAT-ID and normalized name-plus-full-billing-address uniqueness per tenant and Legal Entity, every duplicate path returns the same neutral conflict response, customer responses embed unique establishment links with encrypted local contact data, and `/v1/customer-establishments` provides tenant-safe CRUD without exposing organizational-unit data (US-003).
