...
- hardened the Legal Entity and establishment rollout so site and assignment creation reject inactive domain links, customer-establishment writes require active Customer, Legal Entity, and establishment records, scoped customer readers receive only effectively visible sites and current customer-establishment links, employee and site writes reject legacy organizational-unit input, domain lookups support the corresponding write permissions without widening scoped access, restored customer-establishment links clear omitted local contact data, and factories safely reuse soft-deleted links.
