...
- normalized `POST /v1/auth/token` device-name handling so surrounding whitespace is trimmed and blank values fall back to the documented `api-client` default, making Android/native token naming more explicit without changing the endpoint shape
