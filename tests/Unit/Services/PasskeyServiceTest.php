...
describe('PasskeyService credential deserialization key casing', function () {
    test('client_data_json casing behavior is covered through public PasskeyService APIs', function () {
...
describe('PasskeyService native Android origin support', function () {
    test('allowed origins include the canonical Android passkey origin derived from the signing certificate fingerprint', function () {
        ...
        $fingerprint = (string) config('android.signing_certificate_sha256_fingerprint');
        $normalized = strtolower(str_replace(':', '', $fingerprint));
        $binaryHash = hex2bin($normalized);

        expect($binaryHash)->not->toBeFalse();

        $base64UrlHash = rtrim(strtr(base64_encode($binaryHash), '+/', '-_'), '=');
        $androidOrigin = 'android:apk-key-hash:'.$base64UrlHash;

        expect((array) config('passkeys.allowed_origins'))->toContain('https://app.secpal.dev')
            ->and($androidOrigin)->toBe('android:apk-key-hash:w-n9B2nzNJuwsFa65mlHI0DhyyhmJt4wycn6-V8eR7U');
    });
});
