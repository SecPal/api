...
grep -q '^PUT https://ghcr.io/v2/secpal/api/manifests/' "$tmp_dir/success/curl.log"
...
assert_no_extended_match '^PUT ' "$tmp_dir/target-same/curl.log"

run_case target-different target-different failure
assert_no_extended_match '^PUT ' "$tmp_dir/target-different/curl.log"

for status in 401 403 429 500 503; do
  run_case "target-$status" "target-$status" failure
  assert_no_extended_match '^PUT ' "$tmp_dir/target-$status/curl.log"
done
